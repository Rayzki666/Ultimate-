...
The repository now has a reproducible Capacitor 8 configuration. GitHub Actions uses a macOS runner to:
...
2. **Native feedback** — haptic confirmation when Frame recommends shooting.
...
A signed device or TestFlight build needs an Apple Developer Program team, bundle identifier, signing certificate, and provisioning profile. No signing credentials are required for the current simulator build.
